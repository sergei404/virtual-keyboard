...
let inglishLetters = [[['`', '~'], [1, '!'], [2, '@'], [3, '#'], [4, '$'], [5, '%'], [6, '^'], [7, '&'], [8, '*'], [9, '('], [0, ')'], ['-', '_'], ['=', '+'], 'Backspace'], ['Tab', 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', ['[', '{'], [']', '}'], ['\\', '|'], 'Delete'], ['CapsLock', 'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', [';', ':'], ['\'', '"'], 'Enter'], ['ShiftLeft', 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', ['/', '?'], 'ArrowUp', 'ShiftRight'], ['ControlLeft', 'Meta', 'Alt', 'Space', 'Alt', 'ControlRight', 'ArrowLeft', 'ArrowDown', 'ArrowRight']]
  ...
  return array.map(it => `<div class="keybord-section__row">${it.map(el => {
    if (Array.isArray(el)) {
      ...
      if (el.length === 1) {
        ...
        btn = `<button id=${el} class="keybord-section__btn keybord-section__btn--service">&uarr;</button>`
      } else if (el === 'ArrowLeft') {
        btn = `<button id=${el} class="keybord-section__btn keybord-section__btn--service">&larr;</button>`
